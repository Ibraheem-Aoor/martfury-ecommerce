                                            ...
                                            <td>
                                                ...
                                                <div class="ps-product--cart">
                                                    ...
                                                    <div class="ps-product__content">
                                                        <a
                                                            href="{{ $product->original_product->url }}">{{ $product->name }}</a>
                                                        @if (is_plugin_active('marketplace') && $product->original_product->store->id)
                                                            <p class="d-block mb-0 sold-by"><small>{{ __('Sold by') }}:
                                                                    <a
                                                                        href="{{ $product->original_product->store->url }}">{{ $product->original_product->store->name }}</a></small>
                                                            </p>
                                                        @endif
                                                ...
                                                <div
                                                    ...
                                                    <span>
                                                        @if ($product->pricePerQty != null)
                                                            {{ format_price($product->getProductPricePerQty($cartItem->qty)) }}
                                                        @else
                                            ...
                                            <td class="text-center">
                                                <div class="form-group--number product__qty">
                                                    ...
                                                    <input type="text" class="form-control qty-input"
                                                        value="{{ $cartItem->qty }}" title="{{ __('Qty') }}"
                                                        name="items[{{ $key }}][values][qty]" readonly>
                                                </div>
                                            </td>
                                            <td class="text-center">
                                                @if ($product->pricePerQty != null)
                                                {{ format_price($product->getProductPricePerQty($cartItem->qty) * $cartItem->qty) }}
                                                @else
                    ...
                    <div class="col-lg-6 col-md-12 col-sm-12 ">
                        <div class="ps-block--shopping-total">
                            <div class="ps-block__header">
                                <p>{{ __('Subtotal') }} <span>
                                        {{ format_price(Cart::instance('cart')->rawSubTotal()) }}</span></p>
                            </div>
                            @if (EcommerceHelper::isTaxEnabled())
                                <div class="ps-block__header">
                                    <p>{{ __('Tax') }} <span>
                                            {{ format_price(Cart::instance('cart')->rawTax()) }}</span></p>
                                </div>
                            @endif
                            @if ($couponDiscountAmount > 0 && session('applied_coupon_code'))
                                <div class="ps-block__header">
                                    <p>{{ __('Coupon code: :code', ['code' => session('applied_coupon_code')]) }}
                                        (<small><a class="btn-remove-coupon-code text-danger"
                                            ...
                                            {{ format_price($couponDiscountAmount) }}</span></p>
                                </div>
                            @endif
                        ...
                        <a class="ps-btn ps-btn btn-cart-button-action"
                            href="{{ route('public.checkout.information', OrderHelper::getOrderSessionToken()) }}">{{ __('Proceed to checkout') }}
