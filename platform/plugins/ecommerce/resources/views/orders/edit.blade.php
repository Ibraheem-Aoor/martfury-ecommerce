...
@section('content')
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        <td class="text-end color-subtext mt10">
                                                            ...
                                                            <p class="mb0 font-size-12px">
                                                                {{ ecommerce_convert_weight($weight) }}
                                ...
                                <div class="flexbox-grid-default flexbox-flex-wrap flexbox-align-items-center">
                                    @if ($order->status == \Botble\Ecommerce\Enums\OrderStatusEnum::CANCELED)
                                        <div class="flexbox-auto-left">
                                            <svg class="svg-next-icon svg-next-icon-size-24 svg-next-icon-gray">
                                                <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#next-error">
                                                </use>
                                            </svg>
                                        </div>
                                    ...
                                    @elseif ($order->payment->id)
                                        ...
                                        @if (!$order->payment->status ||
                                            in_array($order->payment->status, [\Botble\Payment\Enums\PaymentStatusEnum::PENDING]))
                                            <div class="flexbox-auto-left">
                                                                        ...
                                                                        <tbody>
                                                                            ...
                                                                            @if ($order->payment->user->getFullName())
                                                                                <tr>
                                                                                    ...
                                                                                    <td>{{ $order->payment->user->getFullName() ? $order->payment->user->getFullName() : trans('plugins/ecommerce::order.n_a') }}
                                                                                    </td>
                                                                                </tr>
                                                                            @endif
                                                                            <tr>
                    ...
                    <div class="ui-layout__item">
                        ...
                        <div class="wrapper-content bg-gray-white mb20">
                            <div class="pd-all-20">
                                ...
                                @if (!in_array($order->status, [
                                    ...
                                    \Botble\Ecommerce\Enums\OrderStatusEnum::COMPLETED,
                                ]))
                        ...
                        <div id="extra-shipment-info" class="widget meta-boxes">
                            <div class="widget-title">
                                <h4><span>Additional shipment information</span></h4>
                            </div>
                            <div class="widget-body">
                                <form id="shipping_info" action="{{ route('orders.company_info.update') }}">
                                    <input type="text" hidden name="order_id" value="{{encrypt($order->id)}}">
                                    <div class="form-group mb-3">
                                        <label for="shipping_company_name"  class="control-label">Shipping Company
                                            Name</label>
                                        <input type="text" name="shipping_company_name" id="shipping_company_name"
                                            class="form-control" placeholder="Ex: DHL, AliExpress..." value="{{$order->shipping_company_name}}">
                                    </div>
                                    <div class="form-group mb-3">
                                        <label for="tracking_id" class="control-label">Tracking ID</label>
                                        <input type="text" name="tracking_id" id="tracking_id" class="form-control" value="{{$order->shipping_tracking_id}}"
                                            placeholder="Ex: JJD0099999999">
                                    </div>
                                    <div class="form-group mb-3">
                                        <label for="tracking_link" class="control-label">Tracking Link</label>
                                        <input type="text" name="tracking_link" id="tracking_link" value="{{$order->shipping_tracking_link}}"
                                            class="form-control"
                                            placeholder="Ex: https://mydhl.express.dhl/us/en/tracking.html#/track-by-reference">
                                    </div>
                                    <div class="form-group mb-3">
                                        <label for="estimate_date_shipped" class="control-label">Estimate Date
                                            Shipped</label>
                                        <input type="text" name="estimate_date_shipped" id="estimate_date_shipped"
                                            class="form-control datepicker" data-date-format="yyyy/mm/dd" value="{{$order->estimate_arrival_date}}"
                                            placeholder="yyyy/mm/dd">
                                    </div>
                                    <div class="form-group mb-3">
                                        <label class="control-label">Note</label>
                                        <textarea class="form-control" name="note" rows="2"  placeholder="Add note..." style="height: 51px;">{{$order->note}}</textarea>
                                    </div>
                                    <div class="form-group mb-3">
                                        <button type="button" onclick="$('#shipping_info').submit();"
                                            class="btn btn-info">Update
                                            Shipping Company Info</button>
                                    </div>
                                </form>
    ...
    @if ($order->status != \Botble\Ecommerce\Enums\OrderStatusEnum::CANCELED)
        ...
        {!! Form::modalAction(
            ...
            trans('plugins/ecommerce::order.cancel_shipping_confirmation'),
            'info',
        ...
        {!! Form::modalAction(
            ...
            'info',
            view('plugins/ecommerce::orders.refund.modal', [
                'order' => $order,
            ...
            trans('plugins/ecommerce::order.refund') .
                ...
                format_price($order->payment->amount - $order->payment->refunded_amount) .
                '</span>',
        ) !!}
        @if ($order->shipment && $order->shipment->id)
            {!! Form::modalAction(
                ...
                'info',
                view('plugins/ecommerce::orders.shipping-status-modal', [
                    'shipment' => $order->shipment,
    ...
    {{-- Start Shipping Company Info form --}}
    <script>
        $('#shipping_info').on('submit', function(e) {
            e.preventDefault();
            var form = $(this);
            $.ajax({
                headers: {
    ...
    {{-- End Shipping Company Info form --}}
@stop
