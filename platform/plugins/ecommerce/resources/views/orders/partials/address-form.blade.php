<div class="customer-address-payment-form">
                        ...
                        @if ($isAvailableAddress)
                            @foreach ($addresses as $address)
                                <option value="{{ $address->id }}"
                                    ...
                                    {{ $address->address }}, {{ $address->city }}, {{ $address->state }}@if (count(EcommerceHelper::getAvailableCountries()) > 1)
    ...
    <div class="address-form-wrapper" @if (auth('customer')->check() &&
            $isAvailableAddress &&
            ((!empty($sessionAddressId) && $sessionAddressId !== 'new') || empty(Arr::get($sessionCheckoutData, 'state')))) style="display: none;" @endif>
        <div class="row">
            <div class="col-12">
            ...
            <div class="col-lg-4 col-12">
                <div class="form-group  @if ($errors->has('address.phone')) has-error @endif">
                    <input type="text" name="address[phone]" id="address_phone"
                        placeholder="{{ __('Phone') }} {{ EcommerceHelper::isPhoneFieldOptionalAtCheckout() ? __('(optional)') : '' }}"
                        class="form-control address-control-item {{ !EcommerceHelper::isPhoneFieldOptionalAtCheckout() ? 'address-control-item-required' : '' }} checkout-input"
            ...
            <div class="col-6">
                <div class="form-group  @if ($errors->has('address.house_no')) has-error @endif">
                    <input id="house_no" type="text"
                        class="form-control address-control-item address-control-item-required checkout-input"
                        placeholder="{{ __('House No') }}" name="address[house_no]"
                        value="{{ old('address.house_no', Arr::get($sessionCheckoutData, 'house_no')) }}">
