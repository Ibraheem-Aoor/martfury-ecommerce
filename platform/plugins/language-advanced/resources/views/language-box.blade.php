    ...
    <div id="list-others-language">
        @foreach($languages as $language)
            @if ($language->lang_code != $currentLanguage->lang_code)
                ...
                <a href="{{ Route::has($route['edit']) ? Request::url() . ($language->lang_code != Language::getDefaultLocaleCode() ? '?ref_lang=' . $language->lang_code : null) : '#' }}" target="_blank">{{ $language->lang_name }} <i class="fa fa-external-link"></i></a>
                <br>
            @endif
        @endforeach
    </div>
